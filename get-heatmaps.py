...
from fastai.vision import ImageDataBunch, ResizeMethod, Image, imagenet_stats
import torch
...
def show_heatmap(hm, path, y, idx, xb_im, only_heatmap=False, interpolation='bilinear', alpha=0.5):
...
def generate_heatmaps(path_img, path_hms=None, alpha=0.5, learn=None):
    if path_img is None:
        return
    ...
    if path_hms is not None and not os.path.exists(path_hms):
        os.mkdir(path_hms)

    if not os.path.exists(path_img/'train'):
        os.mkdir(path_img/'train')
    if not os.path.exists(path_img/'train'/'img'):
        os.mkdir(path_img/'train'/'img')
    ...
    for idx in range(len(temp.train_ds)):
        ...
        if m is not None:
            ...
            show_heatmap(avg_acts, path_hms, y, idx, xb_im, only_heatmap=False, interpolation='spline16', alpha=alpha)
...
if __name__ == '__main__':
    ...
    generate_heatmaps(path_img, path_hms, alpha, learn)
