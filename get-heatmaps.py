...
def hooked_backward(m, xb, cat=None):
...
def show_heatmap(hm, xb_im, y, idx, path, only_heatmap=False, interpolation='bilinear', alpha=0.5):
...
def generate_heatmaps(path_base, path_img, path_hms=None, alpha=0.5):
    path = path_base
    ...
    try:
        ...
        for idx in range(len(temp.train_ds)):
            ...
            hook_a,hook_g = hooked_backward(m, xb, cat=y)
            acts  = hook_a.stored[0].cpu()
            avg_acts = acts.mean(0)

            save_img(x, y, idx, path_hms)
            show_heatmap(avg_acts, xb_im, y, idx, path_hms, only_heatmap=False, interpolation='spline16', alpha=alpha)

    finally:
        # deleting dummy directories and moving back files to where they were
        for file in files:
            source_file = path_img/'train'/'img'/file
            if os.path.exists(source_file):
                os.rename(source_file, path_img/file)
        if os.path.exists(path_img/'train'):
...
if __name__ == '__main__':
    ...
    alpha     = args.alpha
    generate_heatmaps(path_base, path_img, path_hms, alpha)
